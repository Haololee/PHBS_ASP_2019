...
class ModelHagan:
   ...
   '''
        ...
        if(is_vol):
            ...
            for i in range (3):
            vol3[i] = self.bsm_model.impvol(price_or_vol3[i], strike3[i], spot, texp, cp_sign=cp_sign)
        ...
        solroot = sopt.root(func,np.array([0,0,0]))
        solutionx = solroot.x
      
        sigma = np.exp(solution[0])
        alpha = np.exp(solution[1])
        rho = np.tanh(solution[2])
        
        return sigma, alpha, rho # sigma, alpha, rh
...
class ModelNormalHagan:
    ...
    '''Given option prices or normal vols at 3 strikes, compute the sigma, alpha, rho to fit the data
        If prices are given (is_vol=False) convert the prices to vol first.m
        Then use multi-dimensional root solving 
        you may use sopt.root
...
class ModelBsmMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1,fix_seed=True):
        ...
        znorm_m = np.random.normal(size=(self.n_samples, self.time_steps, 2))
        w1 = znorm_m[:, :, 0]
        w2 = self.rho * znorm_m[:, :, 0] + np.sqrt(1-self.rho ** 2)*znorm_m[:, :, 1]
        sigma_tk1 = np.exp(self.alpha * np.sqrt(delta_t) * w2 - 0.5 * self.alpha**2 * delta_t)
        sigma_tk = self.sigma * np.cumprod(sigma_tk1, axis=1)
        forward_tk = forward[:, np.newaxis]*np.cumprod(np.exp(sigma_tk * np.sqrt(delta_t)*w1 - 0.5*sigma_tk**2*delta_t), axis=1)
        pricepayoff = np.mean(np.fmax(forward_tk - strike[:, np.newaxis], 0), axis=1)
        return pricepayoff
'''
...
class ModelNormalMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1,fix_seed=True):
        ...
        sigma_tk1 = np.exp(self.alpha * np.sqrt(delta_t) * w2 - 0.5 * self.alpha**2 * delta_t)
        sigma_tk = self.sigma * np.cumprod(sigma_tk1, axis=1)
        forward_tk = forward[:, np.newaxis]*np.cumprod(sigma_tk*np.sqrt(delta_t)*w1), axis=1)
        pricepayoff = np.mean(np.fmax(forward_tk - strike[:, np.newaxis], 0), axis=1)          
        return pricepayoff
...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1,fix_seed=True):
        ...
        forward = spot / disc_fac * div_fac
        delta_t = texp / self.time_steps
        znorm_m = np.random.normal(size=(self.n_samples, self.time_steps))
        w1 = znorm_m
        sigma_tk1 = np.exp(self.alpha * np.sqrt(delta_t) * w1 - 0.5 * self.alpha**2 * delta_t)
        sigma_tk = self.sigma * np.cumprod(sigma_tk1, axis=1)
        I_T = np.sum(sigma_tk**2*delta_t,axis=1)
        S_t = forward*np.exp((self.rho / self.alpha * (sigma_tk[:, -1] - sigma_tk[:, 0]))-(self.rho**2/2*I_T))
        sigma_t = np.sqrt((1 - self.rho**2)*I_T/ texp)
        CBS = [np.mean(self.bsm_model.price(k, S_t, texp, sigma_t, cp_sign)) for k in strike]
        return np.array(CBS)
...
class ModelNormalCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1,fix_seed=True):
        ...
        sigma_tk = self.sigma * np.cumprod(sigma_tk1, axis=1)
        I_T = np.sum(sigma_tk**2*delta_t,axis=1)
        S_t = forward + self.rho / self.alpha * (sigma_tk[:, -1] - sigma_tk[:, 0])
        sigma_t = np.sqrt((1 - self.rho**2)*I_T/ texp)
        CBS = [np.mean(self.bsm_model.price(k, S_t, texp, sigma_t, cp_sign)) for k in strike] 
        return np.array(CBS)