...
def basket_price_norm_analytic(
    ...
    disc_fac = np.exp(-texp*intr)
    forward = spot / disc_fac * div_fac
    basketforward = weights @ forward #Turning single forward price to a matrix form. 
    '''
    2. compute the normal volatility of basket
    '''
    cov_m = vol * cor_m * vol[:,None] 
    basketvol = np.sqrt(weights.T @ cov_m @ weights)
    '''
    ...
    PUT YOUR CODE BELOW
    '''
    price = disc_fac*normal_formula(strike, basketforward, basketvol, texp, intr=0.0, divr=0.0, cp_sign=cp_sign)
    return price
